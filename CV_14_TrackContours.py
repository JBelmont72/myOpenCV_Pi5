...
how to track on object in openCV based on color, create contours for the object of interests, and then track the object of interest by drawing a bounding box around it. 

https://youtu.be/IupUw5ATllw
...
def onTrack8(val):
    ...
    hueHigh2=val
    print('High Hue2: ',val)
# frame shape  (600, 800, 3) , fps: 17 from svga 
width=800
height = 600
...
while True:
    ...
    # frame=cv2.resize(frameFlip, (width, height))
    frame=cv2.resize(frameFlip, (400, 300))
    # frame=cv2.resize(frameFlip, (int(width/2), int(height/2)))
    print(frame.shape)
 ...
 # myMask is the amalgam of myMask and MyMask2
    ...
    cv2.moveWindow('My Selection',int(width/2),int(height/2+100))
 
    cv2.imshow('My Mask', myMaskSmall)
    # cv2.moveWindow('My Mask',0,int(height+100))
    cv2.moveWindow('My Mask',0,int(height/2+100))
    cv2.imshow('my WEBcam',frame)
    cv2.moveWindow('my WEBcam',300,50)
