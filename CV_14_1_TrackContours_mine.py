...
def onTrack8(val):
    ...
    hueHigh2=val
    print('High Hue2: ',val)
# frame size with ESP32s3 set at vga
width=640
height=480
...
while True:
 ...
 #comment out the above cv2.drawContoure(frame....)
    ...
    mySelection=cv2.bitwise_and(frame,frame, mask=myMask)
    mySelection=cv2.resize(mySelection,(int(width/2),int(height/2)))
    cv2.putText(frame,str(fps) +' fps',(30,60),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,255),1)
    cv2.imshow('My Selection', mySelection)
    ...
    cv2.imshow('my WEBcam',frame)
    cv2.moveWindow('my WEBcam',width,int(100))
    if cv2.waitKey(1) & 0xff ==ord('q'):
        break
cam.release()